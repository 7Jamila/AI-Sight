...
//
//  Created by Jamila Abouelenin on 10/11/24.
...
public class StreamingScanner {
    ...
    public func play(column: Int) {
        ...
        for (row, node) in sourceNodes[0..<Int(numRows)].reversed().enumerated() {
            ...
            if (id == 0 && depth < 1) {
                Logger().error("[\(xCoord), \(yCoord)]: id=\(id), depth=\(depth), volume=\(node.volume)]")
            }
        }
    }
    ...
    func computeVolume(id: Int, depth: Float, yCoord: Int) -> Float {
        ...
        // Calculate base volume based on linear mapping of depth value
        let volume = depth < minDepth ? maxVolume : (
            (minVolume * (depth - minDepth) + maxVolume * volumeCurve) / ((depth - minDepth) +
                volumeCurve)
        ) * equalizerFactor
        
        //Determine object category we're in
        ...
        //For All Close Objects, calculate volume directly using depth ensuring all closer objects are louder
        case "All close objects":
            return depth < Float(UserDefaults.standard.double(forKey: .scannerMaxDepth)) ?
                maxVolume - (depth / Float(UserDefaults.standard.double(forKey: .scannerMaxDepth)))
                * (maxVolume - minVolume) : minVolume
        
        // For Hazard Detection, make far objects quieter exponentially
        case "Hazard Detection":
            let k: Float = 0.9
            let volume_adj = depth < minDepth ? maxVolume : maxVolume * exp(-k * (depth - minDepth))
            return volume_adj
        
        default:
            //For any other category, use the standard volume calculation based on depth and ID
            return objectCategoryIds[objectCategory]?.contains(id) == true ? volume : minVolume
        }
    }
}
